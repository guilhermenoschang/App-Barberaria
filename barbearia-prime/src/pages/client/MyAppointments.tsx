...
export default function MyAppointments() {
  const { appointments, sessionUser, updateAppointmentStatus, config } = useApp()
  const nav = useNavigate()

  // SECURITY: show only own session appointments
  const myAppts = sessionUser
    ? appointments.filter(a => a.sessionId === sessionUser.id || a.clientName === sessionUser.name)
    : []
  ...
  function cancel(id: string) {
    if (confirm('Cancelar este agendamento?')) updateAppointmentStatus(id, 'cancelled')
  }
