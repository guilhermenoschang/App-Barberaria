...
function AdminGuard({ children }: { children: React.ReactNode }) {
  ...
  if (authLoading) return <div style={{ padding: 32, textAlign: 'center' }}>Carregando sessão...</div>
  return isAdmin ? <>{children}</> : <Navigate to="/admin/login" replace />
}

export default function App() {
  const { theme, authLoading } = useApp()

  if (authLoading) {
    return (
      <div data-theme={theme} style={{ minHeight: '100vh', background: 'var(--bg0)', color: 'var(--t1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        Carregando sessão...
      </div>
    )
  }
